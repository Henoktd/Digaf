...
const shareholderImportDryRun_1 = require("../services/shareholderImportDryRun");
...
function createEmptyImportIndex() {
    return {
...
function addIndexValue(index, value) {
...
function normalizeDryRunRows(value) {
...
function deriveBatchStatus(result) {
...
async function persistImportRowsAndMessages(client, input) {
...
async function fetchImportBatchDetail(queryable, batchId) {
...
function requireNoProductionDataConfirmation(value) {
    if (value !== true) {
        throw new Error("confirmNoProductionData must be true before persisting an import batch");
    }
}
exports.importRoutes.post("/shareholders/dry-run", async (req, res) => {
    let rows = [];
    try {
        (0, validation_1.normalizeActorId)(req.body?.actorId);
    ...
    try {
        ...
        const result = (0, shareholderImportDryRun_1.validateShareholderImportDryRun)(rows, existingIndex);
        return res.json({
            data: {
                ...result,
                requestedBy: {
        ...
        return (0, apiError_1.sendServerError)(res, "Failed to run shareholder import dry-run validation", error);
    }
});
exports.importRoutes.post("/shareholders/batches", async (req, res) => {
    let actorId = "";
    let rows = [];
    let entityId = null;
    ...
    try {
        ...
        entityId =
            req.body?.entityId === undefined || req.body?.entityId === null || req.body?.entityId === ""
                ? null
                : (0, validation_1.requireUuid)(req.body.entityId, "entityId");
        sourceFilename =
            req.body?.sourceFilename === undefined || req.body?.sourceFilename === null
                ? null
                : (0, validation_1.requireNonEmptyString)(req.body.sourceFilename, "sourceFilename");
    }
    catch (error) {
        return (0, apiError_1.sendBadRequest)(res, error instanceof Error
            ? error.message
    ...
    try {
      ...
      `, [
        ...
        const detail = await fetchImportBatchDetail(pool_1.pool, batchId);
        ...
        await client.query("ROLLBACK");
        return (0, apiError_1.sendServerError)(res, "Failed to create shareholder import batch", error);
    }
    finally {
        client.release();
    }
});
exports.importRoutes.get("/shareholders/batches", async (_req, res) => {
    try {
    ...
    `);
        ...
        return (0, apiError_1.sendServerError)(res, "Failed to fetch shareholder import batches", error);
    }
});
exports.importRoutes.get("/shareholders/batches/:batchId", async (req, res) => {
    let batchId = "";
    try {
        batchId = (0, validation_1.requireUuid)(req.params.batchId, "batchId");
    }
    catch (error) {
        return (0, apiError_1.sendBadRequest)(res, error instanceof Error ? error.message : "Invalid import batch request");
    }
    try {
        const detail = await fetchImportBatchDetail(pool_1.pool, batchId);
        if (!detail) {
            return (0, apiError_1.sendNotFound)(res, "Import batch not found");
        }
        return res.json({
            data: detail,
        });
        ...
        return (0, apiError_1.sendServerError)(res, "Failed to fetch shareholder import batch", error);
    }
});
exports.importRoutes.post("/shareholders/batches/:batchId/revalidate", async (req, res) => {
    let batchId = "";
    let actorId = "";
    try {
        batchId = (0, validation_1.requireUuid)(req.params.batchId, "batchId");
    ...
    try {
      ...
      ORDER BY source_row_number ASC, created_at ASC
      `, [batchId]);
        const sourceRows = sourceRowsResult.rows.map((row) => row.source_payload_json);
        if (sourceRows.length === 0) {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendBadRequest)(res, "Import batch has no rows to revalidate");
        }
        const existingIndex = await fetchExistingImportIndex();
        const result = (0, shareholderImportDryRun_1.validateShareholderImportDryRun)(sourceRows, existingIndex);
        const batchStatus = deriveBatchStatus(result);
        await client.query(`
      ...
      `, [batchId, batchStatus, JSON.stringify(result.summary)]);
        ...
        await client.query("ROLLBACK");
        return (0, apiError_1.sendServerError)(res, "Failed to revalidate shareholder import batch", error);
    }
    finally {
        client.release();
    }
});
exports.importRoutes.post("/shareholders/batches/:batchId/submit-review", async (req, res) => {
    let batchId = "";
    let actorId = "";
    try {
        batchId = (0, validation_1.requireUuid)(req.params.batchId, "batchId");
    ...
    try {
      ...
      `, [batchId]);
        if ((errorResult.rows[0]?.error_count ?? 0) > 0) {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendBadRequest)(res, "Import batch cannot be submitted for review while validation errors remain");
        }
        await client.query(`
      UPDATE shareholder_import_batches
      SET
      ...
      `, [batchId]);
        ...
        await client.query("ROLLBACK");
        return (0, apiError_1.sendServerError)(res, "Failed to submit shareholder import batch for review", error);
    }
    finally {
        client.release();
    }
});
exports.importRoutes.post("/shareholders/messages/:messageId/resolve", async (req, res) => {
    let messageId = "";
    let actorId = "";
    let resolutionStatus = "";
    let resolutionNotes = null;
    try {
        messageId = (0, validation_1.requireUuid)(req.params.messageId, "messageId");
        actorId = (0, validation_1.normalizeActorId)(req.body?.actorId);
        resolutionStatus = (0, validation_1.requireNonEmptyString)(req.body?.resolutionStatus, "resolutionStatus");
        resolutionNotes =
            req.body?.resolutionNotes === undefined ||
                req.body?.resolutionNotes === null ||
                req.body?.resolutionNotes === ""
                ? null
                : (0, validation_1.requireNonEmptyString)(req.body.resolutionNotes, "resolutionNotes");
    }
    catch (error) {
        return (0, apiError_1.sendBadRequest)(res, error instanceof Error
            ? error.message
    ...
    if (!["accepted", "resolved", "waived", "rejected"].includes(resolutionStatus)) {
        return (0, apiError_1.sendBadRequest)(res, "resolutionStatus must be one of: accepted, resolved, waived, rejected");
    }
    const roleResult = (0, roles_1.requireRole)(req.body?.actorRole, [
        "maker",
        "checker_2",
    ...
    try {
      ...
      `, [messageId]);
        if (messageResult.rowCount === 0) {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendNotFound)(res, "Import validation message not found");
        }
      ...
      `, [messageId, resolutionStatus, actorId, resolutionNotes]);
        await insertImportBatchEvent(client, {
            ...
            eventType: resolutionStatus === "waived"
        ...
        await client.query("COMMIT");
        const detail = await fetchImportBatchDetail(pool_1.pool, message.batch_id);
        return res.json({
            data: detail,
        });
    }
        ...
        await client.query("ROLLBACK");
        return (0, apiError_1.sendServerError)(res, "Failed to resolve shareholder import validation message", error);
    }
    finally {
        client.release();
    }
});
exports.importRoutes.post("/shareholders/rows/:rowId/exclude", async (req, res) => {
    let rowId = "";
    let actorId = "";
    let reviewNotes = null;
    try {
        rowId = (0, validation_1.requireUuid)(req.params.rowId, "rowId");
        actorId = (0, validation_1.normalizeActorId)(req.body?.actorId);
        reviewNotes =
            req.body?.reviewNotes === undefined ||
                ...
                ? null
                : (0, validation_1.requireNonEmptyString)(req.body.reviewNotes, "reviewNotes");
    }
    catch (error) {
        return (0, apiError_1.sendBadRequest)(res, error instanceof Error
            ? error.message
    ...
    try {
      ...
      `, [rowId]);
        if (rowResult.rowCount === 0) {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendNotFound)(res, "Import row not found");
        }
      ...
      `, [rowId, actorId]);
        ...
        await client.query("COMMIT");
        const detail = await fetchImportBatchDetail(pool_1.pool, row.batch_id);
        return res.json({
            data: detail,
        });
    }
        ...
        await client.query("ROLLBACK");
        return (0, apiError_1.sendServerError)(res, "Failed to exclude shareholder import row", error);
    }
    finally {
        client.release();
    }
