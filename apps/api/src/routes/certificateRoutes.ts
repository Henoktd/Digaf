    ...
    res.json({
      data: {
        ...
        render_metadata: {
          ...
          disclaimer:
            "PDF generation is not implemented yet. This payload is certificate-safe render data for future PDF template generation.",
        },
      },
    });
  } catch (error) {
  ...
  <article class="certificate" aria-label="Demo share certificate">
    <header class="topline">
      <div>
        ...
        <p class="subtitle">Registered issuer: ${escapeHtml(
          certificate.issuing_company
        )}</p>
      </div>
      <div class="serial">
