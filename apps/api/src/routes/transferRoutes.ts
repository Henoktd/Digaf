...
type EligibilityShareholderRow = {
...
function normalizePositiveShares(value: unknown) {
  const shares =
    typeof value === "number"
      ? value
      : typeof value === "string"
...
function normalizeSupportingDocuments(value: unknown) {
...
async function buildTransferEligibility(
  ...
  const shareholdersById = new Map<string, EligibilityShareholderRow>(
    shareholderResult.rows.map((row: EligibilityShareholderRow) => [
      row.shareholder_id,
      row,
    ])
  );
  ...
  const freezeCounts = new Map(
    freezeResult.rows.map((row: any) => [row.shareholder_id, row.active_count])
  );
  const transferorFreezeActive =
    Number(freezeCounts.get(input.transferorId) ?? 0) > 0;
  const transfereeFreezeActive =
    Number(freezeCounts.get(input.transfereeId) ?? 0) > 0;
  ...
  const legalHoldCounts = new Map(
    legalHoldResult.rows.map((row: any) => [
      row.shareholder_id,
      row.active_count,
    ])
  );
  const transferorLegalHoldActive =
    Number(legalHoldCounts.get(input.transferorId) ?? 0) > 0;
  const transfereeLegalHoldActive =
    Number(legalHoldCounts.get(input.transfereeId) ?? 0) > 0;
...
transferRoutes.post("/:transferId/cancel", async (req, res) => {
  ...
  try {
    ...
    return sendBadRequest(
      res,
      error instanceof Error ? error.message : "Invalid transfer cancellation"
    );
  }
  ...
  try {
    ...
    if (!transfer) {
      ...
      return sendNotFound(res, "Share transfer not found");
    }

    if (
      !["draft", "pending_checker_1", "pending_checker_2"].includes(
        transfer.status
      )
    ) {
      await client.query("ROLLBACK");
      return sendConflict(res, "Share transfer cannot be cancelled");
    }

    if (actorRole === "maker" && actorId !== transfer.maker_id) {
      await client.query("ROLLBACK");
      return sendForbidden(res, "Maker can only cancel their own transfer");
    }
    ...
    return sendServerError(res, "Failed to cancel transfer", error);
  } finally {
    client.release();
  }
});
...
transferRoutes.post("/", async (req, res) => {
  let input;
  let supportingDocuments: unknown[] = [];
  ...
  try {
    ...
    if (!eligibility.eligible) {
      await client.query("ROLLBACK");
      return sendBadRequest(res, "Transfer is not eligible", {
        blockingReasons: eligibility.blockingReasons,
      });
    }

    const encumbranceCheckStatus =
      eligibility.warnings.length > 0 ? "warning" : "passed";
    ...
    return sendServerError(res, "Failed to create share transfer", error);
  } finally {
    client.release();
  }
});
