...
exports.approvalRoutes.post("/:approvalId/reject", async (req, res) => {
    let approvalId = "";
    let actorId = "";
    let decisionNotes = "";
    try {
        approvalId = (0, validation_1.requireUuid)(req.params.approvalId, "approvalId");
        ...
        return (0, apiError_1.sendBadRequest)(res, error instanceof Error ? error.message : "Invalid approval rejection");
    }
    ...
    try {
      ...
      `, [approvalId]);
        ...
        if (!approval) {
            ...
            return (0, apiError_1.sendNotFound)(res, "Approval request not found");
        }
        ...
        if (!isChecker1Stage && !isChecker2Stage) {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendConflict)(res, "Approval request is not at a rejectable stage");
        }
        if (isChecker1Stage &&
            actorRole !== "checker_1" &&
            actorRole !== "governance_admin") {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendForbidden)(res, "Only Checker 1 or Governance Admin can reject at Checker 1 review");
        }
        if (isChecker2Stage &&
            actorRole !== "checker_2" &&
            actorRole !== "governance_admin") {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendForbidden)(res, "Only Checker 2 or Governance Admin can reject at Checker 2 review");
        }
        if (actorId === approval.maker_id) {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendConflict)(res, "Maker cannot reject their own request");
        }
        if (isChecker2Stage && actorId === approval.checker1_id) {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendConflict)(res, "Checker 1 cannot reject as Checker 2");
        }
      ...
      `, [approval.reference_id]);
        ...
        if (!transfer) {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendNotFound)(res, "Linked share transfer not found");
        }
        const oldValue = {
            approval: {
                stage: approval.stage,
      ...
      `, [
        ...
        await client.query("ROLLBACK");
        return (0, apiError_1.sendServerError)(res, "Failed to reject approval", error);
    }
    finally {
        client.release();
    }
...
exports.approvalRoutes.post("/:approvalId/approve-checker-1", async (req, res) => {
    let approvalId = "";
    let actorId = "";
    let decisionNotes = "";
    try {
    ...
    try {
      ...
      `, [approvalId]);
        ...
        if (approval.stage !== "checker_1_review") {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendConflict)(res, "Approval request is not at Checker 1 review");
        }
        if (actorId === approval.maker_id) {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendConflict)(res, "Maker cannot approve their own request");
        }
        const oldValue = {
            stage: approval.stage,
            status: approval.status,
      ...
      `, [
        ...
        await client.query("ROLLBACK");
        return (0, apiError_1.sendServerError)(res, "Failed to approve Checker 1", error);
    }
    finally {
        client.release();
    }
});
exports.approvalRoutes.post("/:approvalId/approve-checker-2", async (req, res) => {
    let approvalId = "";
    let actorId = "";
    let decisionNotes = "";
    try {
    ...
    try {
      ...
      `, [approvalId]);
        ...
        if (approval.stage !== "checker_2_review") {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendConflict)(res, "Approval request is not at Checker 2 review");
        }
        if (actorId === approval.maker_id) {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendConflict)(res, "Maker cannot approve their own request");
      ...
      `, [approval.reference_id]);
        ...
        if (transfer.status !== "pending_checker_2") {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendConflict)(res, "Share transfer is not pending Checker 2");
        }
        if (transfer.kyc_check_status !== "passed") {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendConflict)(res, "KYC check has not passed");
        }
        if (transfer.encumbrance_check_status !== "passed") {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendConflict)(res, "Encumbrance check has not passed");
        }
        if (transfer.freeze_reference !== null) {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendConflict)(res, "Share transfer is blocked by a freeze reference");
        }
        if (transfer.board_approval_required && !transfer.board_approval_ref) {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendConflict)(res, "Board approval reference is required");
        }
      ...
      `, [transfer.transferor_id, transfer.shares]);
        ...
        if (!transferorOwnership) {
            await client.query("ROLLBACK");
            return (0, apiError_1.sendConflict)(res, "Transferor does not have enough available shares");
        }
      ...
      `, [approvalId]);
        ...
        await client.query("ROLLBACK");
        return (0, apiError_1.sendServerError)(res, "Failed to approve Checker 2", error);
    }
    finally {
        client.release();
    }
